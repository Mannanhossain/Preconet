...
class SuperAdminDashboard {
    ...
    renderStats() {
        ...
        const statsConfig = [
            { label: 'Total Admins', value: this.stats.total_admins ?? 0, icon: 'users-cog', bgColor: 'bg-blue-50', textColor: 'text-blue-600' },
            { label: 'Total Users', value: this.stats.total_users ?? 0, icon: 'users', bgColor: 'bg-green-50', textColor: 'text-green-600' },
            { label: 'Active Admins', value: this.stats.active_admins ?? 0, icon: 'user-check', bgColor: 'bg-green-50', textColor: 'text-green-600' },
            { label: 'Expired Admins', value: this.stats.expired_admins ?? 0, icon: 'exclamation-triangle', bgColor: 'bg-red-50', textColor: 'text-red-600' }
        ];
        ...
        if (logoutBtn) {
            logoutBtn.addEventListener('click', (e) => {
                ...
                if (confirm('Are you sure you want to log out?')) {
                    sessionStorage.removeItem('super_admin_token');
                    sessionStorage.removeItem('super_admin_user');
                    auth.showNotification('You have logged out successfully!', 'success');
                    setTimeout(() => {
                        window.location.href = '/login.html';
                    }, 800);
                }
...
// ✅ Ensure page loads only when DOM ready
document.addEventListener('DOMContentLoaded', () => {
    new SuperAdminDashboard();
});
